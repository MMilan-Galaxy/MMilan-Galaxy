class Quete6 extends Quest {
  ...
  draw(p) {
    ...
    if (this.status === "WON") {
      ...
      this._drawUI(
        p,
        "LE PONT A ÉTÉ RÉPARÉ ET VOUS AVEZ RÉUSSI À LIVRER LE COLIS",
      );
      if (!this._finishing) {
  ...
  _drawBridge(p) {
    ...
    if (this.bridgeRepaired) {
      ...
      for (const h of this.holes) {
        if (currentX < h.x)
          p.rect(currentX, this.groundY, h.x - currentX, bridgeH);
        currentX = h.x + h.w;
      }
      if (currentX < bridgeEnd)
        p.rect(currentX, this.groundY, bridgeEnd - currentX, bridgeH);
    }
  }
  ...
  _handleInput(level) {
    const speakThresh = 0.004;
    const loudThresh = 0.009;
    if (level > loudThresh) {
  ...
  _checkCollisions(p) {
    ...
    if (this.beaverY >= this.groundY - 20) {
      if (
        this.beaverX > this.cliffWidth &&
        this.beaverX < p.width - this.cliffWidth
      ) {
        let isOverHole = false;
        for (const h of this.holes) {
          if (this.beaverX - 2 > h.x && this.beaverX - 2 < h.x + h.w)
            isOverHole = true;
        }
        if (isOverHole) {
  ...
  _drawUI(p, msg) {
    ...
    const panelY = p.height * 0.1;
    dsPanel(p, panelX, panelY, panelW, panelH, {
      cut: 16,
      fill: "#0e0e1a",
      stroke: "#1e1e3a",
      alpha: 220,
    ...
    if (isStart) {
      ...
      p.textSize(Math.min(p.width * 0.02, 20));
      p.text(
        "Parler doucement pour marcher et fort pour sauter",
        p.width / 2,
        panelY + titleSz * 4.2,
      );
    }
